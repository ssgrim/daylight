...
export function getSeasonFor(lat, date = new Date(), mode) {
  const envMode = (process && process.env && process.env.SEASON_MODE) || mode || 'meteorological'
  const month = date.getUTCMonth() + 1
  const northSeason = envMode === 'astronomical' ? northSeasonAstronomical(date) : northSeasonMeteorological(month)
  const hemisphere = lat < 0 ? 'south' : 'north'
  const season = lat < 0 ? (
    northSeason === 'winter' ? 'summer' :
    northSeason === 'summer' ? 'winter' :
    northSeason === 'spring' ? 'autumn' : 'spring'
  ) : northSeason
