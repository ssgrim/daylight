...
export async function initDb() {
  try {
    ...
    try {
      fs.openSync(DB_PATH, "a").close();
    } catch (e) {
      /* ignore */
    }
    const db = await open({ filename: DB_PATH, driver: sqlite3.Database });
    await db.exec(
      `CREATE TABLE IF NOT EXISTS history (id INTEGER PRIMARY KEY, ts TEXT, type TEXT, provider TEXT, lat REAL, lng REAL, ok INTEGER, error TEXT)`,
    );
    return db;
  } catch (err) {
...
export async function appendDb(db, entry) {
  await db.run(
    "INSERT INTO history (ts,type,provider,lat,lng,ok,error) VALUES (?,?,?,?,?,?,?)",
    entry.ts,
    entry.type,
    entry.provider,
    entry.lat,
