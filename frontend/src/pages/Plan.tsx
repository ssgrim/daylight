...
type Suggestion = {
  ...
  events?: {
    provider: string;
    events: Array<{ name: string; venue?: string; date?: string }>;
  };
        ...
        setSuggestions([
          {
            ...
            score: 90,
            reason: `fallback: ${String(err.message ?? err)}`,
          },
        ]);
      })
      ...
      {loading ? (
        ...
        <div>
          <p className="text-yellow-600">
            Unable to load live suggestions ({error}) — showing demo results.
          </p>
            ...
            {suggestions.map((s) => (
              <li key={s.id} className="p-3 border rounded">
                <div className="font-medium">
                  {s.title}{" "}
                  <span className="text-sm text-slate-500">({s.score})</span>
                </div>
          ...
          {suggestions.map((s) => (
            <li key={s.id} className="p-3 border rounded">
              <div className="flex items-center justify-between">
                ...
                {s.season && (
                  <div
                    className={`px-2 py-0.5 rounded text-xs ${s.season.season === "summer" ? "bg-yellow-200 text-yellow-800" : s.season.season === "winter" ? "bg-sky-200 text-sky-800" : "bg-slate-100 text-slate-800"}`}
                  >
                    {s.season.season}
                  </div>
                )}
              ...
              {showTraffic && s.traffic && s.traffic.congestion != null && (
                <div className="mt-2 text-sm">
                  Traffic congestion:{" "}
                  <span className="font-semibold">{s.traffic.congestion}%</span>
                </div>
              )}
            </li>
          ))}
