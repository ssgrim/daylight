  ...
  return (
    <div className="bg-white rounded-lg border shadow-sm p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`px-2 py-1 rounded-full text-xs font-medium border ${getStateColor(stats.state)}`}
        >
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          <div className="font-medium text-green-600">
            {formatTime(stats.lastSuccessTime)}
          </div>
        </div>
        <div>
          <span className="text-gray-500">Last Failure:</span>
          <div className="font-medium text-red-600">
            {formatTime(stats.lastFailureTime)}
          </div>
        </div>
      </div>
      ...
      {stats.state === "OPEN" && stats.nextAttemptTime && (
        <div className="mt-3 p-2 bg-orange-50 rounded text-sm">
          <span className="text-orange-700">
            Next retry attempt: {getNextAttemptIn()}
          </span>
        </div>
      )}
      ...
      {stats.state === "HALF_OPEN" && (
        <div className="mt-3 p-2 bg-yellow-50 rounded text-sm">
          <span className="text-yellow-700">
            Testing service recovery ({stats.successCount} successes needed)
          </span>
        </div>
      )}
    </div>
...
function StatusSummary({
  ...
  summary: CircuitBreakerStatus["summary"];
  status: string;
}) {
...
export default function CircuitBreakerDashboard() {
  ...
  const fetchStatus = async () => {
    ...
    if (!response.ok) {
      throw new Error(
        `HTTP ${response.status}: Failed to fetch circuit breaker status`,
      );
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(data.circuitBreakers).map(([name, stats]) => (
                  <CircuitBreakerCard
                    key={name}
                    name={name}
                    stats={stats as CircuitBreakerStats}
                  />
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-gray-500">
                No circuit breakers configured yet. They will appear here once
