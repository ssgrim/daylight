...
variable "provider_config" {
  description = "Configuration for the pluggable provider system"
  type = object({
    default_provider        = optional(string, "google-places")
    enable_failover        = optional(bool, false)
    timeout_ms             = optional(number, 8000)
    
    google_places = optional(object({
      timeout_ms = optional(number, 8000)
      enabled    = optional(bool, true)
    }), {})
    
    mock_provider = optional(object({
      fail_rate   = optional(number, 0)
      delay_ms    = optional(number, 100)
  ...
  validation {
    condition = contains([
      "google-places",
      "mock"
    ], var.provider_config.default_provider)
    error_message = "Default provider must be one of: google-places, mock."
  }
}
